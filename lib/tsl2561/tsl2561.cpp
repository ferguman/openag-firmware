...
uint8_t SensorTsl2561::begin() {
  ...
  read_register_timeout_ = 10;               // milliseconds

  return status_level;
}

uint8_t SensorTsl2561::update() {
...
uint8_t SensorTsl2561::get() {
  ...
  if (read_register_error_) {
    ...
    status_msg =  "\"GERR 4\":\"tsl2561 read register timeout\",";
    status_level = ERROR;
    lux_ = 0;
  }
...
void SensorTsl2561::writeRegister(int deviceAddress, int address, uint8_t val) {
  ...
  i2c_transmission_status = Wire.endTransmission();     // end transmission
  //delay(100);
}
...
int SensorTsl2561::cmd(int args) {
   ...
   if (this->is_cmd(args, read)) {
      Serial.print(F("LUX: ")); Serial.println(lux_);
      Serial.print(F("PAR: ")); Serial.println(par_);
      return make_int(OK);
   }
